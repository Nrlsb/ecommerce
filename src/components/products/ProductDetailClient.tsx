...
export default function ProductDetailClient({ productId, initialProduct }: ProductDetailClientProps) {
    ...
    useEffect(() => {
        const fetchProductAndVariants = async () => {
            // Si ya tenemos el producto y variantes en caché, no hacemos nada
            if (productCache.has(productId) && variantsCache.has(productId)) {
                return;
            }

            try {
                // Si no tenemos initialProduct o queremos refrescar
                let currentProduct = product || initialProduct;
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-start">
                        ...
                        >
                            ...
                            {product.imagen_url ? (
                                <Image 
                                    ...
                                    width={600}
                                    height={600}
                                    className="w-full h-full object-contain p-12 group-hover:scale-105 transition-transform duration-1000 ease-out" 
                                    priority
                                />
                            ) : (
                    ...
                    <div className="flex flex-col">
                        ...
                        <VariantSelector currentProductId={productId} variants={variants} />
                        ...
                        <div className="glass p-10 rounded-[3.5rem] border border-white/40 dark:border-white/5 mb-12 shadow-premium relative overflow-hidden">
                            ...
                            <div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-8 mb-10 relative">
                                ...
                                <div className="flex items-center bg-white/50 dark:bg-slate-950/50 backdrop-blur-md border border-white/20 dark:border-white/5 rounded-3xl p-2 shadow-sm">
                                    <button type="button" onClick={() => setQuantity(Math.max(1, quantity - 1))} className="w-12 h-12 flex items-center justify-center hover:bg-primary/5 rounded-2xl transition-all font-display font-bold text-xl active:scale-90">-</button>
                                    <span className="w-16 text-center font-display font-black text-2xl tracking-tighter">{quantity}</span>
                                    <button type="button" onClick={() => setQuantity(quantity + 1)} className="w-12 h-12 flex items-center justify-center hover:bg-primary/5 rounded-2xl transition-all font-display font-bold text-xl active:scale-90">+</button>
                                </div>
                            </div>
                        ...
                        {product.yield > 0 && (
                            <ProductCalculator defaultYield={product.yield} onApplyQuantity={(qty) => setQuantity(qty)} />
                        )}
                    </div>
                </div>
